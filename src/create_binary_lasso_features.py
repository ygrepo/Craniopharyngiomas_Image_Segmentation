...
def get_args():
    ...
    ap.add_argument(
        ...
        type=float,
        default=0.20,
        help="Fraction of samples to assign to test set.",
    )
    ap.add_argument(
        "--output_dir",
        type=Path,
...
def main():
    ...
    if train_latent_dir.exists():
        ...
        train_count = len(latent_rows)
        logger.info(f"Loaded {train_count} latent vectors from imagesTr")
    ...
    if test_latent_dir.exists():
        for npy_file in sorted(test_latent_dir.glob("*.npy")):
            ...
            for i, v in enumerate(vec):
                row[f"Latent_{i}"] = float(v)
            latent_rows.append(row)
        test_count = len(latent_rows)
        logger.info(f"Loaded {test_count} latent vectors from imagesTr")
    ...
    df_train.to_csv(
        args.output_dir / f"{args.model_type}_train_binary.csv", index=False
    )
    df_test.to_csv(args.output_dir / f"{args.model_type}_test_binary.csv", index=False)
    logger.info(
    ...
    save_npz(
        args.output_dir / f"{args.model_type}_train_binary_scaled.npz",
        X_train_scaled,
        y_train_bin,
        feature_cols,
    )
    save_npz(
        args.output_dir / f"{args.model_type}_test_binary_scaled.npz",
        X_test_scaled,
        y_test_bin,
        feature_cols,
    )
