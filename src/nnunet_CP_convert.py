...
def remap_labels_to_0123(arr: np.ndarray) -> np.ndarray:
    """BraTS-style remap {0,1,2,4} -> {0,1,2,3}."""
...
def convert_to_nnUNet(
    ...
    *,
    ...
    """
    logger.info(f"{dataset_id} | Starting CP→nnU-Net conversion")
    ...
    for p in all_files:
        ...
        if tag is None:
            unknown.append(p)
            continue
        logger.debug(f"Found {cid}:{tag} -> {p}")
    ...
    labels = OrderedDict({"background": 0, "tumor": 1})
    logger.info(f"Labels: {labels}")
    ds = OrderedDict(
        [
            ("name", dataset_name),
            ("description", f"CP; channels={list(modalities)}"),
            ("reference", "Local"),
            ("licence", "Research"),
            ("release", "1.0"),
            ("tensorImageSize", "3D"),
